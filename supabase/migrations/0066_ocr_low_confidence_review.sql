...
begin
  ...
  delete from public.case_input_findings
   where owner_id=p_owner and case_id=p_case and case_input_id=p_input
     ...
     and finding_code in ('OCR_URL','OCR_INSTITUTION','OCR_PRODUCT','OCR_NUMBER','OCR_NEGATION','OCR_TEXT');
  ...
  for page in select * from jsonb_array_elements(p_pages) loop
    ...
    for field in select * from jsonb_array_elements(page->'low_confidence_fields') loop
      if coalesce(field->>'field_kind','') not in ('URL','INSTITUTION','PRODUCT','NUMBER','NEGATION','TEXT')
        ...
        or exists(select 1 from jsonb_array_elements(field->'bbox') value where value::numeric<0)
        or coalesce((field->>'end')::integer,0)>octet_length(page->>'text')
...
begin
  ...
  if jsonb_typeof(p_selected) is distinct from 'array' or jsonb_array_length(p_selected) not between 1 and 8 then raise exception '선택 항목 수 오류' using errcode='23514'; end if;
