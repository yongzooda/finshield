-- PC-008·N-OPS-003: 완료 Case의 가입 후 마스킹 문서도 기존 비용 원장으로 예약한다.
-- Provider·금액·정산·미확정 예약 유지 규칙은 변경하지 않는다.
...
begin
 ...
 if p_run is not null then
   ...
   -- 탈퇴 접수와 입력 비용 예약이 같은 소유자 잠금을 공유한다.
   perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('account-delete:'||p_owner::text,0));
   if exists(select 1 from public.deletion_requests where owner_id=p_owner and target_type='ACCOUNT' and status<>'COMPLETED') then
     raise exception 'ACCOUNT_DELETING' using errcode='55000';end if;
   if not exists(select 1 from public.case_inputs i join public.financial_cases c on c.id=i.case_id
     ...
     and ((i.input_purpose='PROPOSAL' and c.lifecycle in ('DRAFT','INPUT_REVIEW','NEED_MORE_INFORMATION'))
       or (i.input_purpose='AFTERCARE' and i.input_type in ('IMAGE','PDF') and c.enrollment_confirmed_at is not null
         and exists(select 1 from public.evidence_passports p where p.case_id=c.id and p.owner_id=p_owner)))) then
