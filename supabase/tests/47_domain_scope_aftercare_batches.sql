...
begin
 select id into strict old_id from private.execution_manifests where manifest_version='finshield-p0-loan-v13';
 select id into strict new_id from private.execution_manifests where manifest_version='finshield-p0-loan-v14';
 ...
 if exists(select 1 from private.execution_manifest_agents ma join private.agent_definitions a on a.id=ma.agent_definition_id
  where ma.execution_manifest_id=new_id and (a.version<>'p0-v6' or a.prompt_version not like '%-v6')) then raise exception '새 프롬프트 버전 불일치'; end if;
 if exists(select 1 from private.execution_manifest_agents ma join private.agent_definitions a on a.id=ma.agent_definition_id
  where ma.execution_manifest_id=old_id and a.version<>'p0-v5') then raise exception '과거 실행 정의 변경'; end if;
 ...
 if (select count(*) from private.agent_tool_allowlists l join private.agent_definitions a on a.id=l.agent_definition_id where a.version='p0-v6')
  <> (select count(*) from private.agent_tool_allowlists l join private.agent_definitions a on a.id=l.agent_definition_id where a.version='p0-v5') then raise exception 'Agent 도구 허용 목록 누락'; end if;
 raise notice '47_domain_scope_aftercare_batches: 새 버전·일곱 Agent·허용 도구·과거 정책 보존 통과했습니다';
end $$;
rollback;
