...
export function PrivacyActions() {
  ...
  const signOut = async () => {
    if (!token || signingOut || busy) return;
    ...
    try {
      ...
      if (response.ok && body?.status === "SIGNED_OUT") {
        setRows(null); setAsking(null); setNotice(null); setToken(null);
  ...
  return (
    <>
      <FsCard>
        ...
        {sessionNotice ? <p role="status" className="fs-body mt-2">{sessionNotice}</p> : null}
        <button type="button" disabled={signingOut || busy} onClick={() => void signOut()} className="fs-btn fs-btn--quiet mt-4">
                  ...
                  {deleting ? (
                    ...
                    <div className="mt-3 rounded-[10px] bg-[var(--fs-canvas)] px-4 py-3">
                      ...
                      <div className="mt-3 flex flex-wrap gap-3">
                        <button type="button" disabled={busy || signingOut} onClick={() => void requestDelete(row.id)}
