...
import {
  ...
  heartbeat, loadConfirmedClaims, startRevalidation,
...
export async function POST(
  ...
  const sql = fsql();
  const claims = await loadConfirmedClaims(sql, ownerId, caseId);
  if (claims.length === 0) {
    return jsonNoStore({ error: "다시 확인할 항목이 없습니다" }, 409);
  }
  ...
  const progress: RunProgress = (event) => {
    push(event);
    void heartbeat(sql, job.jobId, job.leaseToken);
  };
