...
export async function uploadFile(args:{file:File;supabaseUrl:string;publishableKey:string;objectPath:string;token:string;
  ...
  const headers={"Tus-Resumable":"1.0.0",Authorization:`Bearer ${args.token}`,apikey:args.publishableKey,"x-upsert":"false"};
  ...
  const started=await fetch(endpoint,{method:"POST",headers:{...headers,"Upload-Length":String(args.file.size),
  ...
  while(offset<args.file.size) {
    ...
    try {
      const response=await fetch(url,{method:"PATCH",headers:{...headers,"Content-Type":"application/offset+octet-stream","Upload-Offset":String(offset)},
      ...
      const status=await fetch(url,{method:"HEAD",headers,signal:args.signal,redirect:"error"});
