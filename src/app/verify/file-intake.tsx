...
export type PreparedFile={case_id:string;input_id:string;claims:FileClaim[];masked_text:string;masked_pages:{page_no:number;text:string}[]};

export function FileIntake({token,onPrepared,onBusyChange}:{token:string;onPrepared:(value:PreparedFile)=>void;onBusyChange:(busy:boolean)=>void}) {
  ...
  return <section className="mt-7 border-t border-[var(--fs-line)] pt-5" aria-label="문서로 입력">
    ...
    <label className="mt-4 block fs-body" htmlFor="proposal-file">권유 문서 선택</label>
