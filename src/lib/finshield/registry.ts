...
import { AGENTS, DEFINITION_VERSION, MANIFEST_VERSION, TOOLS } from "./manifest";
...
export const loadManifest = async (sql: Sql): Promise<ResolvedManifest> => {
  ...
  const manifests = await sql`
    select id, kb_release_id from private.execution_manifests
     where manifest_version = ${MANIFEST_VERSION}`;
  if (manifests.length !== 1) {
    throw new ManifestDriftError(`실행 Manifest ${MANIFEST_VERSION} 이 DB 에 없다. Migration 0021 을 적용했는지 확인할 것`);
  }
