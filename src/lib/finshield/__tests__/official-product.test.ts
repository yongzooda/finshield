...
describe("공식 상품의 종료 고지와 수집 신선도 분리", () => {
  ...
  it("새로 읽은 종료 자료는 근거를 보존하면서 확정 인용을 막는다", async () => {
    ...
    expect(result.items[1]).toMatchObject({ isCitable: true, directness: "DIRECT",
      excerptMasked: "햇살론15 [2025년 12월 31일 보증 종료]",
      locator: { permitted_use: "REFUTE_CURRENT_OFFER", current_transaction_proof: false } });
    expect(result.items[1].excerptMasked).not.toContain("15.9%");
  });
});
