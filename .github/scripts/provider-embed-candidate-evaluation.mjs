...
export const FIXTURE_PATH = ".github/fixtures/provider-embed-v3.json";
export const CANDIDATE_POOL_K = 20;
export const FORMULA_VERSION = "candidate-pool-unit-recall-v3";
...
export const expandCandidateFixtures = (fixture) => {
  requireValue(keys(fixture, ["schema_version", "fixture_set", "status", "scope", "split_policy", "cases"])
    ...
    && fixture.fixture_set === "finshield-korean-finance-embed-v3"
    && fixture.status === "preregistered" && text(fixture.scope) && text(fixture.split_policy)
    && Array.isArray(fixture.cases) && fixture.cases.length === 24, "v3 preregistration schema");
  ...
  requireValue(gate.length === 100 && queries.length === 120
    ...
    && new Set(gate.flatMap((query) => query.hard_negative_unit_ids)).size >= 30,
  "gate sample counts");

  // 후보 풀 20 은 corpus 168 unit 의 약 12% 다. 전체를 반환하면 Recall 이
  // 자동으로 1.00 이 되므로 풀이 corpus 보다 충분히 작은지 확인한다.
  requireValue(documents.length === 168 && CANDIDATE_POOL_K * 4 <= documents.length,
    "candidate pool must stay well below corpus size");
