...
- Anthropic Sonnet 5의 auth·quota·schema·strict tool·지연·비용은 `B-MODEL-01` 범위에서 Live 검증됐다. 재정의로 무효가 된 이전 run `33783765337`의 결과 파일도 이력으로 보존한다. Cohere `embed-v4.0`, CLOVA OCR, 공공데이터·법제처 API는 아직 Live 검증되지 않았다.
