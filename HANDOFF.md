...
- Provider Implementation Gate (`N-QLT-010`): `NO-GO` — `B-MODEL-01`·`B-EMBED-01`·`B-SOURCE-02`·`B-SOURCE-03`·`B-FILE-SAFETY`·`B-SUPABASE-01`이 `PASS`이고 나머지 14개가 미해제다. `GO` 전환에는 Claim 판정 품질 평가셋 사전등록도 필요하다.
...
- [x] `B-RATE-01` 동시 예약·정산 원장·Rate·Provider 직렬화 증거 harness 와 main 전용 workflow
