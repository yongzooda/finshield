## 2026-09-12 공식 근거 갱신 예약 중지

- `source-refresh.yml` 의 `schedule` 을 지우고 수동 실행만 남겼다. 예방 공지 점검 job 도 이때 함께 돈다. 되살릴 때 쓸 직전 값은 `41 */3 * * *` 이고 [운영 기준](docs/ops/source-refresh-schedule.md)에 남겼다.
- 미국 실행 환경에서 공공데이터포털·진흥원 접속이 `UND_ERR_CONNECT_TIMEOUT` 으로 끊겨 2026-09-11 오후부터 예약 실행 네 번이 잇달아 실패했다. 같은 주소를 한국에서 부르면 정상이라 코드 문제가 아니다.
- 공개 Demo 를 보여 주기 전에는 `gh workflow run source-refresh.yml --ref main` 을 한 번 돌려 24시간 신선도를 확보한다. 2026-09-12T07:06Z 수동 실행이 성공해 2026-09-13T07:06Z 까지 신선하다.

## 2026-09-11 동의 격리·파일 안전 증거 재채택

- 일반 호칭 오차단 수정(PR #331)과 휴대폰 사진 끝 구조 수정(PR #334)으로 되돌렸던 두 증거를 main `9c702ccd` 에서 다시 재 PR #336 에서 채택한다. 부분 PASS 는 13/20 으로 돌아간다.
- B-CONSENT-01 run `34582048537`: 거절·철회 등 7경로의 전송 0건, 허용 1경로 전송 1건, 감사 누락 0건이다. 합성 개인정보 136개는 모두 가리거나 막았고 정상 문장 31개 오차단은 0건이다.
- B-FILE-SAFETY run `34582050894`: Fixture v2 119건 중 위험 입력 93건이 모두 거부되고 정상 20건(휴대폰 사진 구조 포함)이 모두 통과했다. 판정 어긋남 0건이다.
- Implementation `NO-GO`, Release `NOT-EVALUATED` 를 유지한다.
...
- 로그인한 뒤 새 탭을 열면 다시 로그인해야 했다. 새 탭은 세션 식별자로 그 세션의 HttpOnly Refresh Cookie 갱신을 요청해 이어받는다. Token·Refresh 는 탭 공용 저장소에 두지 않고, 로그아웃은 열린 탭 모두에서 풀린다. [세션 갱신 기준](docs/ops/session-refresh.md)을 따른다.
- 파일 거부를 10쪽 초과·보호 설정 PDF·실행 요소·형식 불일치·해상도로 나눠 알린다. 이미지를 OCR 동의 없이 올리면 업로드 전에 알린다. 빈 쪽이 섞인 디지털 PDF 는 동의 없이 글자 층이 있는 쪽만 읽고 빠진 쪽을 알린다.
- 증거 scope 파일은 바꾸지 않았다. 휴대폰 사진 끝의 제조사 부가 정보 거부와 PDF 압축 바이트 오탐은 B-FILE-SAFETY scope 라 별도 PR 에서 재측정한다.

## 2026-09-11 대출 권유 문자의 일반 호칭 오차단 수정

- 심사위원 관점 점검에서 `[Web발신] 고객님 안녕하세요…`, `회원님!`, `(광고) … 고객님,` 처럼 호칭 뒤에 띄어쓰기·문장부호가 오는 권유 문자가 개인정보 잔존 의심으로 막혀 텍스트·파일 검증을 시작도 못 하는 것을 재현했다. 이름 형태 의심 규칙이 일반 호칭 예외를 적용하지 않았고, 9자리 이상 계약·승인번호도 입력 전체를 막았다.
- 일반 호칭(고객·회원·사장 등)은 막지 않고, 그 앞에 붙은 이름은 새 이름 규칙이 가린다(`홍길동 고객님` → `[이름] 고객님`). 9자리 이상 번호는 막지 않고 가리며, 금액(…원·만원·억)은 가리지도 막지도 않는다. `100000000원` 이 연락처로 가려지던 기존 오탐도 고쳤다.
- 합성 개인정보 136건 미검출 0건, 정상 문장(권유 문자 6건 추가) 차단·마스킹 0건이다.
- 이 파일들은 B-CONSENT-01 scope 라 채택 항목을 `evidence/development/runtime/pre-pii-honorific-consent-entry.json` 에 보존하고 `NOT-EVALUATED` 로 되돌렸다. 병합 뒤 main 에서 다시 재 별도 Adoption PR 로 올린다. 부분 PASS 는 재채택 전까지 12/20 이다.
...
- 코드 69415ae7의 PDF 검증은 82.861초에 저장됐고 네 Domain·CoVe·Judge 성공, Red Team 인용 일부 실패 때문에 PARTIAL이다. 정상 예방 Text는 61.599초에 모든 단계 성공·위험 요구 오탐 없음이다. 기본 676개 통과·선택적 96개 skip, 타입·lint 오류 0, 격리 DB 54 Migration·SQL 시험 44까지 통과했다.
- 운영 DB 적용 범위는 0001~0050 및 0054~0057이다. v11·result-matrix-v2와 기존 버전을 함께 보존한다. 다른 Draft의 0051~0053은 미적용이다. 후보 DB·배포 정합성은 통과했으며 main 운영 승격·최종 정리 결과는 후속 기록에서 확인한다.
- [재현 실패 이력과 최종 검증](docs/ops/member-evidence-scope.md)을 따른다. 전체 P0 완료가 아니며 부분 PASS 8/20·Implementation NO-GO·Release NOT-EVALUATED를 유지한다.
