...
격리 DB 63개 Migration·SQL 시험 50파일과 실제 DB 통합 시험 1개가 통과했다. 기본 718개 통과·선택적 96개 생략, 타입·빌드·lint 오류 0건(기존 경고 2건)을 확인했다. UI Fixture에서 미확인 선택 차단·확인 후 선택·편집 후 재확인·부정어 강조를 확인했다. 첫 격리 DB 시험은 로컬 자격 설정과 포트 충돌, NOLOGIN 때문에 시작에 실패했다. 다른 컨테이너를 변경하지 않고 자체 55448 컨테이너를 준비한 후 통과했다. 첫 UI 확인 스크립트는 접근성 ref 정규식 오류로 실패하여 실제 스냅샷 형식에 맞춰 수정했다. 렌더 내부 콜백의 지역 변수 재할당을 거부한 lint도 반복문으로 수정한 뒤 통과했다. 이들은 인식률 재평가 결과가 아니며 기존 F1 미달은 유지한다.
