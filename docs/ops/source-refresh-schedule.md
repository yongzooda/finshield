...
`.github/workflows/source-refresh.yml` 이 3시간마다 main 에서 돈다. 수동 실행도 된다.
이 저장소의 예약 실행은 정한 시각보다 몇 시간 늦게 시작된 적이 있어 처음 6시간 간격을 3시간으로 좁혔다.
...
- 공공데이터 접속 시간 초과처럼 수집이 실패하면 1분 뒤 두 번까지 다시 수집한다. 그래도 실패하면 적재하지 않고 끝나고 3시간 뒤 다시 시도한다.
  24시간 기한 안에 일곱 번 더 시도할 수 있다.
