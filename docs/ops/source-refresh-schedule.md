...
예약 실행을 껐다. `.github/workflows/source-refresh.yml` 은 수동 실행만 받는다.

```bash
gh workflow run source-refresh.yml --ref main
```

한 번 성공하면 그 시각부터 24시간 동안 신선하다. 공개 Demo 를 보여 주기 전에 한 번 실행하고
`gh run list --workflow source-refresh.yml` 로 성공을 확인한다.

예약을 끈 이유는 두 가지다. 첫째, 미국 실행 환경에서 공공데이터포털·진흥원 접속이 자주
`UND_ERR_CONNECT_TIMEOUT` 으로 끊겨 2026-09-11 오후부터 예약 실행 네 번이 잇달아 실패했다.
같은 주소를 한국에서 부르면 정상이므로 코드 문제가 아니다. 둘째, 매일 신선하게 둘 필요가
없어졌다. 예약이 필요해지면 아래 이력을 참고해 `schedule` 을 되살린다(직전 값은 `41 */3 * * *`).
...
- 공공데이터 접속 시간 초과처럼 수집이 실패하면 1분 뒤 두 번까지 다시 수집한다. 그래도 실패하면 적재하지 않고 끝난다.
  예약이 없으므로 다시 실행하는 것은 사람이 판단한다.
...
- 수동 실행의 실패도 GitHub 이 저장소 관리자에게 알린다.
