...
| `change_d1` | 등록 도메인 | 어제 자 변경 조문이 답하는가 |
| `change_d3` | 등록 도메인 | 사흘 전 변경 조문이 답하는가 |
...
| 등록하지 않은 도메인 통과 | `0` |
| 어제·사흘 전 변경 조문 조회 | Preview·Production 둘 다 |
| 시간 초과 | `0` |
| 서버 오류 | `0` |
...
- 변경 조문의 D+1 반영은 어제와 사흘 전 두 날짜의 응답으로 기록한다. 하루를 실제로 기다리지는 않는다. 그날 바뀐 조문이 없으면 목록이 비어 오는 것이 정상이므로, 반영 여부가 아니라 조회가 답하는지를 합격선으로 둔다.
- 일자별 조문 개정 목록의 `target`은 `lsJoHstInf`다. 첫 development 실행에서 다른 값을 써 본문이 비어 왔고 공식 안내로 바로잡았다.
