...
로컬 코드에서 실제 법제처 API에 `금융소비자 보호에 관한 법률 제19조`를 조회해 법률과 시행령의 `제19조` 본문을 각각 복원했다. 조문 번호를 목록 검색어에서는 분리하고 본문 선택에는 유지해 API 0건 오류와 원장 식별자 누락을 함께 막는다. 이는 법령 도구의 연결 확인이며 전체 Demo 성공 증거는 아니다.
