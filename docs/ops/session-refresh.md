...
Access를 탭 저장소에만 두어 로그인한 뒤 새 탭을 열면 다시 로그인해야 했다. 로그인하면 세션 식별자(UUID)만 `localStorage` 의 `finshield_session_hint` 에 남긴다. Token이 없는 새 탭은 Authorization 없이 `X-FinShield-Session` 헤더로 이 식별자를 보내고, 서버는 그 세션 이름의 Refresh Cookie가 하나 있을 때만 발급처 회전을 요청한다. 발급처가 돌려준 세션이 식별자와 다르면 Cookie를 바꾸지 않는다. 같은 Origin 검사·Web Locks 직렬화·만료 시 Cookie 삭제는 기존 갱신과 같다.

식별자만으로는 아무것도 열 수 없다. Cookie가 없거나 만료되면 401 이고 표시를 지운다. 로그아웃·만료는 그 세션의 표시만 지워 다른 탭의 새 로그인 표시는 남긴다. 확인하는 동안(최대 8초) 보호 화면은 로그인 카드를 먼저 띄우지 않는다. 이미 열린 탭은 다른 탭에서 로그인하면 다음 확인(보이는 상태에서 30초마다) 때 같은 세션을 이어받는다.
