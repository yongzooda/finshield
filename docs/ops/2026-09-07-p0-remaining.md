현재 추가 보안 작업: Migration 0037의 직접 RLS 세션 차단을 실제 검증했다. DB scope 변경 뒤 최종 main에서 다섯 항목을 다시 측정했고 별도 PR #230에서 채택한다. main의 부분 PASS는 13/20이다. 첫 채택 CI 거부와 이전 측정 원본을 보존한다. 세션 갱신은 별도 PR #224에서 실제 1시간 자연 만료 뒤 복구까지 확인했다.
