...
- main은 PR #216의 계정 삭제 선행 보호, #220의 DB 증거 5종 재채택, #218의 OCR 실패 원본 보존을 반영했다. 원격 DB는 0036까지다. 계정 탈퇴 API 전체를 구현한 것은 아니다.
- 기능 실험의 최신 전체 테스트는 494개 통과·90개 선택적 건너뜀이다. 실제 회원 PDF·PNG·10쪽 스캔 처리, 단일 Claim 결과, 재검증 NO_CHANGE, Case 삭제·반복 응답, 가입 후 점검 저장·복원과 삭제 재인증을 확인했다. 성공·실패 원본은 `evidence/development`에 함께 있다.
- Case 삭제는 같은 Origin·최근 5분 비밀번호 인증을 요구한다. 실제 346초가 지난 유효 세션은 조회가 되더라도 삭제를 거부했다. 브라우저는 재로그인 뒤 원래 삭제 확인으로 돌아오며 자동 삭제하지 않는다. 서버가 완료한 삭제는 화면에서도 완료로 안내한다.
- main의 Provider 부분 PASS는 13/20이다. 기능 브랜치는 다른 Next 빌드 설정 때문에 Health를 별도 미평가로 유지한다. Implementation은 NO-GO, Release는 NOT-EVALUATED다. 확정 제출 문서는 보존했다.

남은 필수 작업:

1. OCR은 URL 7필드 오인식으로 F1 0.9791667이 기준 0.98에 미달했다. 실패 원본과 기준을 유지하고 실제 인식 개선·새 사전등록 평가가 필요하다. 기존 URL을 정답으로 임의 치환하지 않는다.
2. 종단 검색은 이전 두 평가에서 기준에 미달했다. 새로운 시나리오 가족의 사전등록 평가, 실제 KB·분쟁 도구 연결과 검색 품질 검증이 필요하다.
3. Workflow의 실제 배포 장애 20종, 연결 단절·취소·기한, Orphan 회수와 Run·Job 종결 원장 일치를 검증해야 한다. 종결 확인 보완과 단일 정상 재검증은 전체 Gate를 대신하지 않는다.
4. 전체 계정 탈퇴는 최근 재인증→새 작업 차단→모든 Case·임시물 정리→Auth 마지막 삭제와 복구까지 남았다. 먼저 적용한 Profile Guard를 우회하거나 완료 상태를 앞당기면 안 된다.
5. Processor 계약·보존·개인정보 미처리 강제와 전체 Text/Image/PDF 조합의 Vertical Spike를 완료해야 한다. Hobby 합성 전용 조건을 유지한다.
6. Implementation GO 뒤 복수 Claim 정상 전체 실행, 실질적 적합성 규칙, Sales/Regulation Agent 기반 가입 후 보호, 일반 상품의 적용 시점, OCR 확인 UX와 알림 복구를 마무리하고 Release 4개 조건을 평가해야 한다.

## 15시 38분: 재검증 실패 원장 동시 종결

- `REV-001`·`N-AVL-008`: Job 실패·Lease 해제·실패 이벤트와 연결된 활성 Run 실패를 같은 DB 트랜잭션으로 묶었다. Job 잠금·Lease 검사를 먼저 거쳐 오래된 Worker가 Run을 종결하지 못하게 한다. Run ID 응답이 유실돼도 Job에 연결된 Run을 DB에서 찾는다.
- 종결 재생은 Run 원장까지 확인한다. 과거 FAILED Job에 남은 활성 Run은 Provider 재호출 없이 복구하고, 성공 Job에 예상 밖의 활성 Run이 있으면 기존 결과를 바꾸지 않고 미확정 오류를 반환한다.
- 실제 격리 PostgreSQL 0036의 `finshield_worker`로 Run 쓰기 경계에 SQL 오류를 주입했다. Job·Run은 RUNNING, Lease·이벤트 수는 원래 값으로 rollback됐다. 잘못된 Lease도 상태를 바꾸지 않았다. 정상 재시도는 두 상태를 FAILED로, Lease를 null로 만들었고 실패 이벤트는 하나였다. 반복 호출·종결 복원은 새 이벤트·Passport를 만들지 않았다. 과거 분리 쓰기 상태도 실제 DB에서 재현·복구했다.
- 관련 단위 시험 18건, 실제 DB 통합 시험 1건, 전체 단위 시험 494건 통과·선택적 90건 건너뜀, TypeScript 통과. 통합 시험은 로컬 주소에만 연결하도록 제한한다. 실제 배포 장애 20종·모든 Lease 경합·계정 삭제/새 Run 생성 경합 검증을 대신하지 않는다.

## 가입 후 점검 실제 저장·복원 검증

- 실제 보호 Preview에서 합성 PDF Case의 가입 사실을 등록하고 계약 문구 차이·답변 네 개·점검 결과·공식 채널을 저장했다. 기존 Passport 두 판은 유지했다. 이는 실제 가입이 아닌 합성 시나리오다.
- 최초 GET에서 `CONTRACT_MATCHES_EXPLANATION` 설문이 문구 비교 prefix로 잘못 분류돼 누락됐다. 실패 원본을 보존하고 실제 Claim UUID 형식의 비교 코드만 분리하도록 고쳤다. 기존 DB 답변은 변경하지 않았다.
- 수정 Preview GET은 같은 점검 ID·기준 Passport·문구 비교·답변 네 개를 복원했고 추가 점검을 만들지 않았다. 로컬 브라우저와 실제 FinShield DB에서도 결과 화면, 편집 양식의 네 선택값을 확인했다. 관련 11시험·TypeScript와 전체 466시험이 통과했다(선택적 89개 건너뜀).
- `evidence/development/aftercare`에 실패와 수정 검증을 보존한다. 규칙·문구 비교의 연결 검증이며 Sales/Regulation Agent나 법적 판단 품질 Gate 완료는 아니다.

## 재검증 종결 확인 보완

- 실패 RPC의 오류를 무시한 뒤 Workflow Step이 고정 FAILED를 반환하던 경로를 수정했다. DB가 아직 QUEUED/RUNNING이거나 확인 조회에 실패하면 비밀 없는 오류로 Step을 재시도한다. DB에 이미 CHANGED/NO_CHANGE/FAILED가 저장돼 있으면 실제 상태를 반환해 응답 유실과 경합을 복원한다.
- 미확정 Provider Run을 발견한 재시도는 기존처럼 모델을 재호출하지 않는다. 이는 종결 응답 계약의 보완이며 Vercel 장애 20종·Orphan 회수 전체 검증 완료는 아니다.
...
- `37f8e9f`의 보호 Preview에서 실제 합성 Case 삭제를 검증했다. 첫 삭제와 두 번의 재요청이 같은 삭제 요청 ID·COMPLETED를 반환하고 Case 조회는 404다. 원본·OCR 임시물은 삭제 전에 실제 부재를 확인한 대상이다.
- 이미 삭제된 Case의 Purge 재호출 오류를 PENDING으로 바꾸던 응답을 고쳤다. 사용자 JWT·RLS로 본인 삭제 요청의 완료 상태를 다시 읽는다. 최초 실패 관측과 수정 후 PNG/스캔 Case 결과를 모두 보존했다.
- 삭제 원장 키는 합성 시험 계정으로 제한된 Preview 브랜치에만 설정했다. Production·계정 탈퇴 전체 완료는 아니다.

### 공식 상품 종료 시점과 수집 신선도 분리

- 실제 공식 안내의 `[2025년 12월 31일 보증 종료]` 고지를 확인했다. 최근 HTTP 수집 시각이 현재 가입 가능한 조건이라는 뜻이 되지 않도록 격리 상품 어댑터를 보완했다.
- 종료일이 지났거나 종료 고지 날짜를 안전하게 읽을 수 없으면 본문·출처를 보존하되 `CONTEXT_ONLY`·확정 인용 불가로 전달한다. 종료일 당일까지의 예정 고지와 수집 `FRESH`는 별도 축으로 유지한다.
- 날짜 경계·잘못된 날짜·미지원 날짜 형식·실제 도구 응답 계약 8개 시험과 타입 검사를 통과했다. 공식 종료 자료로 현재 거래의 상품 조건을 확정하지 않는 보완이며 모든 상품의 시간 유효성이나 과거 시점 조회를 완성한 것은 아니다.
