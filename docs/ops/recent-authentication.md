...
합성 계정의 이미 삭제된 Case를 대상으로 보호 Preview에서 Origin 누락·외부 Origin을 각각 403으로 거부했고, 같은 Origin과 새 비밀번호 인증은 기존 COMPLETED 응답을 복원했다. 실제 346초가 지난 같은 Token은 조회 200을 유지하면서 삭제 요청만 `403 REAUTHENTICATION_REQUIRED`로 거부했다. 앞뒤 기록 목록은 같았다. 결과는 `evidence/development/deletion/2026-09-07-recent-authentication.json`에 있다.

로컬 브라우저와 실제 FinShield DB에서도 오래된 인증의 재로그인 화면, 동일 기록의 삭제 확인 복귀, 자동 삭제 없음과 네 기록 보존을 확인했다. Next 개발 서버가 내부 URL을 localhost로 바꾸는 경우를 재현해 실제 요청 Host와 Origin을 비교하도록 보완했다. 개발용 진단 로그 코드는 제거했다. 보안·삭제 응답 시험 25개와 TypeScript, 수정 UI의 lint를 통과했다.
