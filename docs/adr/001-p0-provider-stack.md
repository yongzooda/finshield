...
| `EVID-SUPABASE-01` | 원격 Schema·RLS·SQL 불변식 | PASS | Migration 0036 뒤 main run `34088194712`, artifact `10005933902`. Migration 36개·SQL 25파일·484개 검증이 통과했다. 원격과 빈 기준 DB의 routine 106개·table 81개·constraint 741개·index 255개 digest가 모두 일치하고 RLS 81개·anon 누출 0건·Worker 거부 13표를 확인했다. 과거 원본은 이력으로 보존한다 |
...
| `EVID-RATE-01` | 예산·Rate·Provider 직렬화 원장 | PASS | Migration 0036 뒤 main run `34088196712`, artifact `10005934380`. 동시 예약 60회에서 초과 허용 0건·정산 불일치 0건, Rate 40회 중 10회 허용·30회 거부, Provider 슬롯 간격 1000ms다. 외부 Provider 호출 없는 격리 DB 계약이다. 과거 원본은 이력으로 보존한다 |
| `EVID-CONSENT-01` | 동의 격리와 원본 전송 경계 | PASS | Migration 0036 뒤 main run `34088198708`, artifact `10005932630`. 거절 7경로 전송 0건·허용 1경로 전송 1건·감사 누락 0건이다. PII 132건 중 117건 마스킹·15건 잔존 차단, 정상 25건 오차단 0건이다. 외부 Provider를 호출하지 않는 prototype 검증이다. 과거 원본은 이력으로 보존한다 |
| `EVID-STORAGE-01` | 인증 사용자 Storage 권한과 발급 token 재사용 | PASS | Migration 0036 뒤 main run `34088200653`, artifact `10005931824`. 실제 회원 JWT로 정상 업로드 1건·차단 8경로를 검증했다. 부당 읽기·쓰기·덮어쓰기·닫힌 token 재사용 허용 0건이며 RLS 우회 키를 사용하지 않았다. 과거 원본은 이력으로 보존한다 |
| `EVID-DELETE-01` | 원본·OCR·Case vector 물리 삭제 | PASS | Migration 0036 뒤 main run `34088202621`, artifact `10006032628`. 합성 40 Case 중 삭제 대상 35건의 원본·OCR·vector와 기발급 URL 접근이 남지 않았다. 조기 만료 5건은 보존했고 최대 삭제 165초, 시험 뒤 객체 잔존 0건이다. 24시간 실제 대기나 실제 Workflow 증거는 아니다. 과거 원본은 이력으로 보존한다 |
