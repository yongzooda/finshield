...
| `EVID-FILE-SAFETY-01` | Parser 이전 검사와 격리 실행 Fixture 행렬 | PASS | main `7ac52bd5` run `34499515046`, artifact `10161269064`, Adoption PR #313. 합성 Fixture 중 위험 입력 82건이 모두 거부되고 정상 15건이 모두 통과했다. 판정 어긋남 0건, Parser 단계까지 간 실행 12건, 최장 6.7초다. 격리는 network namespace와 Node 권한 모델을 함께 쓰며 Parser는 `pdfjs-dist` 6.3.289를 lockfile로 고정했다 |
