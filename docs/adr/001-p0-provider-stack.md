...
| `EVID-LAW-02` | 법제처 OC·등록 도메인 Referer·응답 원장과 Snapshot 해시 | PASS | main `7ac52bd5` run `34499518729`, artifact `10161264574`, Adoption PR #313. Preview와 Production 배포 안의 관측 endpoint로 법제처를 15회 두드렸다. 등록 도메인 Referer 2건, 고정 질의 Snapshot 3건, 일자별 조문 개정 조회 D+1·D+3 각 2건이 통과했고 Referer를 빼거나 미등록 도메인을 보낸 6건은 모두 막혔다. 429와 5xx와 시간 초과는 0건이고 가장 느린 응답은 9,082ms다. 결과에는 요청 주소와 OC와 본문 원문을 담지 않았다 |
| `EVID-MODEL-01` | Anthropic Sonnet 5 Live Spike | PASS | main `7ac52bd5` run `34499501128`, artifact `10161425266`, Adoption PR #313. 합성 50건과 Live 요청 100건이 schema·strict tool·사후 검증을 모두 통과했고 결정적 adapter fault 20건의 거짓 성공은 0건이다. 호출 P95는 3,449ms이고 Text 20건은 P95 6,913ms·USD 0.006418, File-derived 20건은 P95 5,252ms·USD 0.006364다. 의존성 보안 패치로 바뀐 package scope에서 다시 쟀다 |
| `EVID-EMBED-01` | Cohere embed-v4.0 Claim 단위 1차 후보 생성 Live Spike | STALE | Cohere Fast 제품 Retrieval 결정(PR #306)으로 ADR decision digest가 바뀌어 과거 채택(main run `33956964817`, Adoption PR #86)을 재사용하지 않는다. main `7ac52bd5` run `34499504669`에서는 이미 한 번 측정한 v5 holdout의 재측정을 수집기가 거부해 결과를 만들지 않았다. 새로 검토한 미측정 평가셋을 사전등록한 뒤 다시 잰다 |
| `EVID-SOURCE-02` | 공공데이터포털 금융위·진흥원 API key·quota·license Live Snapshot | PASS | main `7ac52bd5` run `34506019177`, artifact `10163852324`, Adoption PR #313. 두 API 결과 코드 `00`, serviceKey 인증 성공, 금융위 총건수 19를 1 page로 전량 수신했다. 같은 main의 첫 실행 run `34499522268`은 공공데이터 접속 시간 초과로 결과를 만들기 전에 멈춰 품질 관측이 없었고 같은 main에서 다시 실행했다. serviceKey는 결과·로그·URL 기록에서 지웠다 |
| `EVID-SOURCE-03` | Demo `햇살론15` 상품·취급기관 Snapshot | PASS | main `7ac52bd5` run `34506182109`, artifact `10163916299`, Adoption PR #313. 금융위 `햇살론15` 19건 중 현재 기준월은 `202602`이고 진흥원 취급기관 16개가 상품명으로 모두 이어졌다. 공식 상품 페이지와 사칭 신고센터는 200으로 도달했고 이용안내 페이지는 실행 환경에서 기본 틀만 돌려줘 도달 실패로 기록했다. 같은 main의 첫 실행 run `34499525297`은 접속 시간 초과로 결과를 만들기 전에 멈췄다 |
| `EVID-SUPABASE-01` | 원격 Schema·RLS·SQL 불변식 | PASS | main `e5d05cae` run `34559818408`, artifact `10183919799`, Adoption PR #324. Migration 68개와 SQL 계약 시험 54파일을 격리 DB 에 적용해 516개 단언이 통과했고 실패는 0개다. 운영 DB 와 schema digest 여섯 개(표·제약·색인·함수·view·정책)가 모두 같다. RLS enable·force 83/83, anon 권한 잔존 0건, 비공개 스키마 노출 0건, 교차 Owner 거부 247건, worker 회원 본문 거부 13표, 예상 밖 허용 0건이다. 앞선 main `7ac52bd5` run `34499535894` 는 시험 14파일의 완료 표시 문구가 수집기 기준과 달라 실패했고 PR #323 이 문구만 고쳤다. 실패 이력은 보존한다 |
| `EVID-FILE-SAFETY-01` | Parser 이전 검사와 격리 실행 Fixture 행렬 | STALE | 휴대폰 사진 끝 구조(Samsung SEFT·MPF gain map) 거부와 PDF 두 글자 key 오탐 수정으로 scope(`file-safety-inspector.mjs`·`file-safety-fixtures.mjs`)가 바뀌어 새 main 측정과 별도 Adoption 전까지 재사용하지 않는다. 다음은 과거 채택 이력이다. main `7ac52bd5` run `34499515046`, artifact `10161269064`, Adoption PR #313. 합성 Fixture 중 위험 입력 82건이 모두 거부되고 정상 15건이 모두 통과했다. 판정 어긋남 0건, Parser 단계까지 간 실행 12건, 최장 6.7초다. 격리는 network namespace와 Node 권한 모델을 함께 쓰며 Parser는 `pdfjs-dist` 6.3.289를 lockfile로 고정했다 |
| `EVID-RETRIEVAL-01` | Metadata Filter·Keyword·Vector·Rerank 종단 top 5 | PASS | main `7ac52bd5` run `34499508015`, artifact `10161418157`, Adoption PR #313. 사전등록한 미노출 v6 평가셋 20가족 100 Claim에서 Metadata Filter·Keyword FTS·Exact KNN·Cohere `rerank-v4.0-fast` 종단 top 5가 Recall@5 1.00, Precision@5 1.00, 위험 핵심 Recall@5 1.00이다. 질의 P95 638ms(Embed 528ms·Fast 275ms·DB 3ms), Provider 요청 203건, 총비용 USD 0.2016으로 상한 USD 0.25 안이다. 앞선 결정적 재정렬의 두 실패 run `34027686263`·`34029362670`은 이력으로 보존한다. 이 측정은 실제 Postgres FTS 경로를 썼다. ADR의 `B-SUPABASE-01` 선행 조건은 main `e5d05cae` run `34559818408`(Adoption PR #324)로 채워졌다 |
| `EVID-RATE-01` | 예산·Rate·Provider 직렬화 원장 | PASS | main `7ac52bd5` run `34499539154`, artifact `10161275772`, Adoption PR #313. 동시 예약 60회에서 상한 초과 허용 0건·정산 불일치 0건이며 Rate 40회 중 10회 허용·30회 거부다. Provider 슬롯 최소 간격은 1,000ms다. 외부 Provider 호출 없는 격리 DB 계약이다 |
| `EVID-CONSENT-01` | 동의 격리와 원본 전송 경계 | STALE | PII 일반 호칭 오차단 수정으로 scope(`src/lib/agents/pii.ts`·`pii-synthetic.ts`)가 바뀌어 새 main 측정과 별도 Adoption 전까지 재사용하지 않는다. 다음은 과거 채택 이력이다. main `7ac52bd5` run `34499542659`, artifact `10161271358`, Adoption PR #313. 거절·철회 등 7경로의 전송 0건·허용 1경로 전송 1건·감사 누락 0건이다. PII 132개 중 마스킹 뒤 잔존 15개는 전송을 차단했고 정상 25개 오차단은 0건이다. 외부 Provider를 호출하지 않은 prototype 검증이다 |
| `EVID-STORAGE-01` | 인증 사용자 Storage 권한과 발급 token 재사용 | PASS | main `7ac52bd5` run `34499545676`, artifact `10161270103`, Adoption PR #313. 실제 회원 업로드 1건과 거부 8경로가 통과했다. 부당 읽기·쓰기·덮어쓰기·닫힌 TUS 재사용 허용은 0건이며 RLS 우회 키를 쓰지 않았다 |
| `EVID-DELETE-01` | 원본·OCR·Case vector 물리 삭제 | PASS | main `7ac52bd5` run `34499548717`, artifact `10161585716`, Adoption PR #313. 합성 40 Case 중 대상 35건의 원본·OCR·Vector·기발급 URL 접근이 남지 않았다. 만료 전 5건은 보존했고 최대 삭제는 224초, 삭제 뒤 인증 읽기는 0건이다. 24시간 전체 대기나 Vercel Workflow 장애 증거는 아니다 |
| `EVID-RUNTIME-01` | Preview·Production 실제 Node·region·deployment | PASS | main `7ac52bd5` run `34499528694`, artifact `10161253403`, Adoption PR #313. 배포 안의 관측 endpoint를 Preview와 Production 배포 3개씩에서 불렀고 여섯 배포 모두 Node 판·region·deployment ID를 돌려줬다. 배포가 적은 deployment ID가 Vercel API 값과 여섯 건 모두 같다. 실제 Node는 v24.19.0이고 region은 icn1이다. 배포 보호는 자동화 우회 비밀로 열었고 그 값은 결과에 남기지 않았다 |
| `EVID-HEALTH-01` | 저비용 Health 실제 HTTP·DB 장애·외부 전송 계측 | PASS | main `7ac52bd5` run `34499532323`, artifact `10161290510`, Adoption PR #313. 실제 Next HTTP 100회에서 외부 fetch·HTTP·HTTPS 시도는 모두 0회다. DB 장애의 기본·엄격 응답은 503이며 계측 제어가 세 경로를 각각 1회 검출했다. 미관측 Provider 상태는 unknown으로 유지하며 제품 전체 정상이나 Provider 품질 증거로 확대하지 않는다 |
| `EVID-OCR-01` | 합성 32문서·112쪽 Parser·OCR 정확도 | FAIL | 사전등록한 v2 평가셋(v1과 겹치지 않는 8가족, 32문서·112쪽)의 정식 1회 측정 main `7ac52bd5` run `34499511820`에서 지원 112/112쪽, 숫자 TP 560·부정 표현 TP 336의 FP/FN 0, 10쪽 P95 16,958ms였다. 필드는 TP 327·FP 9·FN 9로 F1 0.9732이며 기준 0.98에 미달했다. 차이 9쌍은 모두 `earlyrepay` 가족 스캔 PDF의 주소 필드였고 v1 run `34086750192`(F1 0.9792, 스캔 한 가족 URL 7쪽)과 같은 유형이다. 합격선·정답을 바꾸거나 재측정해 유리한 결과를 고르지 않는다 |
...
| `B-RETRIEVAL-01` | Metadata Filter·Keyword FTS·Vector·Rerank 종단 top 5 품질 | PASS | §15.1 종단 Retrieval 합격 + query별 단계 원장; `B-SUPABASE-01` 선행 |
