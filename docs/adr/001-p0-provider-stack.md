...
| `EVID-RATE-01` | 예산·Rate·Provider 직렬화 원장 | PASS | Migration 0035 뒤 main run `34079508301`, artifact `10003235603`. 동시 예약 60회 중 초과 허용 0건, 정산 원장 불일치 0건, Rate 40회 중 10회 허용·30회 거부, Provider 슬롯 최소 간격 1000ms다. 외부 Provider 호출은 없는 격리 DB 계약 검증이다. 과거 결과는 이력으로 보존한다 |
| `EVID-CONSENT-01` | 동의 격리와 원본 전송 경계 | PASS | Migration 0035 뒤 main run `34079510109`, artifact `10003226882`. 거절 7경로 전송 0건, 허용 1경로 전송 1건, 감사 누락 0건이다. PII Fixture 132건 중 117건 마스킹·15건 잔존 차단, 식별자 유출 0건, 정상 25건 오차단 0건이다. 외부 Provider를 호출하지 않는 동의 prototype 검증이다. 과거 결과는 이력으로 보존한다 |
