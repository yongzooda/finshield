...
| `EVID-FILE-SAFETY-01` | Parser 이전 검사와 격리 실행 Fixture 행렬 | PASS | main `9c702ccd` run `34582050894`, artifact `10192029257`, Adoption PR #336. 합성 Fixture v2 중 위험 입력 93건이 모두 거부되고 정상 20건이 모두 통과했다. 판정 어긋남 0건, Parser 단계까지 간 실행 13건, 최장 6.0초다. v2는 휴대폰 사진 끝 구조(Samsung SEFT·MPF gain map)와 PDF 두 글자 key 문법 Fixture를 더했다(PR #334). 격리는 network namespace와 Node 권한 모델을 함께 쓰며 Parser는 `pdfjs-dist` 6.3.289를 lockfile로 고정했다. 이전 채택 이력은 main `7ac52bd5` run `34499515046`, Adoption PR #313이다 |
...
| `EVID-CONSENT-01` | 동의 격리와 원본 전송 경계 | PASS | main `9c702ccd` run `34582048537`, artifact `10192025705`, Adoption PR #336. 거절·철회 등 7경로의 전송 0건·허용 1경로 전송 1건·감사 누락 0건이다. PII 136개 중 마스킹 뒤 잔존 15개는 전송을 차단했고 정상 31개 오차단은 0건이다. 일반 호칭 오차단 수정(PR #331)으로 대출 권유 문자 정상 문장을 더했다. 외부 Provider를 호출하지 않은 prototype 검증이다. 이전 채택 이력은 main `7ac52bd5` run `34499542659`, Adoption PR #313이다 |
