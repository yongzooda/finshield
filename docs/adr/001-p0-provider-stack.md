...
| `EVID-SUPABASE-01` | 전용 Supabase Project Migration·RLS·교차 소유 거부 행렬 | STALE | Migration 0036으로 DB scope가 바뀌어 재측정한다. 과거 main run `34080993706`, artifact `10003687859`. Migration 35개·SQL 시험 24개, 집계 assertion 471건·실패 0건이다. 실제 FinShield와 기준 DB의 표 81개·제약 741개·인덱스 255개·함수 105개 digest가 모두 같다. RLS enable+force 81/81, anon 누출 0, Worker 회원 본문 거부 13/13, 교차 소유 거부 237건, 닫힌 slot·차단 읽기 각각 20회 거부다. 완료 문구 불일치로 실패한 run 34079506106과 과거 증거는 이력으로 보존한다 |
...
| `EVID-RATE-01` | 예산·Rate·Provider 직렬화 원장 | STALE | Migration 0036으로 DB scope가 바뀌어 재측정한다. 과거 Migration 0035 뒤 main run `34079508301`, artifact `10003235603`. 동시 예약 60회 중 초과 허용 0건, 정산 원장 불일치 0건, Rate 40회 중 10회 허용·30회 거부, Provider 슬롯 최소 간격 1000ms다. 외부 Provider 호출은 없는 격리 DB 계약 검증이다. 과거 결과는 이력으로 보존한다 |
| `EVID-CONSENT-01` | 동의 격리와 원본 전송 경계 | STALE | Migration 0036으로 DB scope가 바뀌어 재측정한다. 과거 Migration 0035 뒤 main run `34079510109`, artifact `10003226882`. 거절 7경로 전송 0건, 허용 1경로 전송 1건, 감사 누락 0건이다. PII Fixture 132건 중 117건 마스킹·15건 잔존 차단, 식별자 유출 0건, 정상 25건 오차단 0건이다. 외부 Provider를 호출하지 않는 동의 prototype 검증이다. 과거 결과는 이력으로 보존한다 |
| `EVID-STORAGE-01` | 인증 사용자 Storage 권한과 발급 token 재사용 | STALE | Migration 0036으로 DB scope가 바뀌어 재측정한다. 과거 Migration 0035 뒤 main run `34079512207`, artifact `10003224195`. 실제 시험 계정의 사용자 JWT로 정상 업로드 1건과 차단 8경로를 검증했다. 부당 읽기·쓰기·덮어쓰기·닫힌 token 재사용 허용은 모두 0건이다. RLS 우회 키 없이 측정했다. 과거 결과는 이력으로 보존한다 |
| `EVID-DELETE-01` | 원본·OCR 임시물·Case vector 물리 삭제와 기발급 URL 차단 | STALE | Migration 0036으로 DB scope가 바뀌어 재측정한다. 과거 Migration 0035 뒤 main run `34079528185`, artifact `10003318461`. 합성 40 Case의 확인·중단·삭제·만료 경계를 측정했다. 삭제 대상 35건의 원본·OCR·vector 잔존과 기발급 URL 접근은 0건이다. 조기 만료 5건은 보존됐고 시험 뒤 객체를 모두 정리했다. 24시간 실시간 대기나 실제 Workflow 검증은 아니다. 과거 결과는 이력으로 보존한다 |
