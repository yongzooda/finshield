...
| `EVID-SUPABASE-01` | 원격 Schema·RLS·SQL 불변식 | FAIL | main `7ac52bd5` run `34499535894`에서 불변식 시험 14파일(40~52·54)이 완료 표시를 `통과했습니다` 대신 `통과`로 찍어 수집기가 실패로 판정했다. 해당 SQL 단언은 모두 정상 종료했지만 원격 schema digest 대조 단계에는 가지 않았다. 과거 채택(main run `34119203303`, Adoption PR #230)은 DB scope 변경으로 재사용하지 않는다. 시험 완료 표시를 고친 뒤 새 main에서 다시 잰다 |
...
| `EVID-RETRIEVAL-01` | Metadata Filter·Keyword·Vector·Rerank 종단 top 5 | PASS | main `7ac52bd5` run `34499508015`, artifact `10161418157`, Adoption PR #313. 사전등록한 미노출 v6 평가셋 20가족 100 Claim에서 Metadata Filter·Keyword FTS·Exact KNN·Cohere `rerank-v4.0-fast` 종단 top 5가 Recall@5 1.00, Precision@5 1.00, 위험 핵심 Recall@5 1.00이다. 질의 P95 638ms(Embed 528ms·Fast 275ms·DB 3ms), Provider 요청 203건, 총비용 USD 0.2016으로 상한 USD 0.25 안이다. 앞선 결정적 재정렬의 두 실패 run `34027686263`·`34029362670`은 이력으로 보존한다. 이 측정은 실제 Postgres FTS 경로를 썼고 ADR의 `B-SUPABASE-01` 선행 조건은 같은 창의 Supabase 측정이 시험 완료 표시 문구로 실패해 아직 채워지지 않았다 |
