...
| `EVID-SUPABASE-01` | 전용 Supabase Project Migration·RLS·교차 소유 거부 행렬 | PASS | main run `34080993706`, artifact `10003687859`. Migration 35개·SQL 시험 24개, 집계 assertion 471건·실패 0건이다. 실제 FinShield와 기준 DB의 표 81개·제약 741개·인덱스 255개·함수 105개 digest가 모두 같다. RLS enable+force 81/81, anon 누출 0, Worker 회원 본문 거부 13/13, 교차 소유 거부 237건, 닫힌 slot·차단 읽기 각각 20회 거부다. 완료 문구 불일치로 실패한 run 34079506106과 과거 증거는 이력으로 보존한다 |
