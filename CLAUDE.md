...
- 커밋 제목·본문, squash 병합 메시지, 이슈·PR 제목·본문, 직접 작성하는 댓글은 한국어로 쓴다. `fix`·`test` 등 타입, API·모델명·식별자, 명령어, 원문 로그, `Closes #N` 연결 구문은 원형을 유지한다.
- 커밋 전에 `.githooks/commit-msg`, PR에서 필수 `check`의 한국어 기록 검사를 통과한다. squash 본문은 GitHub이 72열로 다시 줄바꿈하므로 식별자·수치가 긴 문장은 짧게 끊어 각 행에 한국어를 남긴다. 제목만 한국어로 쓰고 본문을 영어로 남기지 않는다. 검사 통과와 별도로 설명 문장의 한국어 여부를 검토한다.
- 이미 공개된 커밋은 메시지 정정을 위해 임의로 재작성하지 않는다. SHA 보존이 승인된 기록은 `docs/ops/korean-record-corrections.md`에 한국어 정정 내용을 남긴다.
